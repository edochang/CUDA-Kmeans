...
__global__ void findNearestCentroidsCudaKernel_shmem(double* points, double* centers, int* labels, int dim, int k, int num_points, int threads) {
    //printf("hello world from block %d and thread %d with dim(%d), k(%d) \n", blockIdx.x, threadIdx.x, dim, k);  // debug statement

    // hypothesis: the most read data here is the centers as each point is compared to each center to find the nearest centroid.  Attempt to store center in shared memory.  Total shared memory per block for a RTX 3060 Laptop GPU is 49152 bytes (from deviceQuery).  80% of this is 39322 bytes.  39322 bytes / 8 bytes (per double) is ~4900 double elements that can be allocated per block.  32*16 center elements are 512 elements.  Assumption is that we will not be handling so many clusters and dimensions (features) for calculating kmean.

    int point_label_index = threadIdx.x + blockIdx.x * threads;
    int local_index = threadIdx.x;

    // Setup shared memory
    int temp_centers_size = k * dim;
    extern __shared__ double temp_centers[];
    
    // setup window
    double window = ceil((double)temp_centers_size / (double)threads);
    
    // populate temp_centers
    int i = 0;
...
__global__ void addLabeledCentroidsCudaKernel_shmem(double* points, double* centers, int* labels, int* cluster_label_count, int dim, int num_points, int threads) {
...
__global__ void averageLabeledCentroidsCudaKernel_shmem(double* centers, int* cluster_label_count, int dim, int k, int threads) {
    ...
    if (clusterId < k) {
        ...
        for (int d = 0; d < dim; d++) {
            centers[cluster_start_index + d] = temp_centers[cluster_start_index + d] / cluster_label_count[clusterId];
        }
    }
}

__global__ void convergenceCudaKernel_shmem(double* centers, double* old_centers, double* convergence_k, int dim, int k, int threads) {
    int clusterId = threadIdx.x + blockIdx.x * threads;
    int local_index = threadIdx.x;
    ...
    double* temp_old_centers = s;
    double* temp_centers = (double *)&temp_old_centers[temp_centers_size];
    ...
    int index_start = local_index * (int)window;
    while(index_start < temp_centers_size && index_start + i < temp_centers_size) {
        temp_old_centers[index_start + i] = old_centers[index_start + i];
        temp_centers[index_start + i] = centers[index_start + i];
        if (++i == window) {
            i = 0;
            break;
    ...
    if (clusterId < k) {
        ...
        for (int d = 0; d < dim; d++) {
            cluster_converge_sum = cluster_converge_sum + ((temp_centers[ci_start + d] - temp_old_centers[ci_start + d]) / temp_old_centers[ci_start + d] * 100);
        }
...
void computeKMeansCudaShmem(dataset_t* dataset) {
    ...
    //while(iteration != 2) {
        ...
        // Kernel launch findNearestCentroidsCudaKernel
        findNearestCentroidsCudaKernel_shmem<<<blocks_points, threads, size_bytes_centers>>>(d_points, d_centers, d_labels, dim, k, num_points, threads);
        ...
        */
        ...
        //cudaEventRecord(start, 0);
        addLabeledCentroidsCudaKernel_shmem<<<blocks_points, threads>>>(d_points, d_centers, d_labels, d_cluster_label_count, dim, num_points, threads);
        ...
        */
        ...
        //cudaEventRecord(start, 0);
        convergenceCudaKernel_shmem<<<blocks_cluster, threads, size_bytes_centers + size_bytes_centers>>>(d_centers, d_old_centers, d_convergence_k, dim, k, threads);
        //cudaEventRecord(stop, 0);
        //cudaEventSynchronize(stop);
        //cudaEventElapsedTime(&memsettime, start, stop);
